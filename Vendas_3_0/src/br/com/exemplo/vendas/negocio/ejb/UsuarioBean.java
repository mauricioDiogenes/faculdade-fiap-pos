...
public class UsuarioBean implements UsuarioRemote, UsuarioLocal
{
	...
	public ServiceDTO inserirUsuario( ServiceDTO requestDTO  ) throws LayerException
	{
		...
		if ( vo != null )
		{
			Usuario usuario = new Usuario( vo.getLogin( ), vo.getSenha( ), vo.getGrupo( ), vo.getPerfil( ), 
										   vo.getBloqueado( ), vo.getUltimoAcesso( ), vo.getNome( ) ) ; 
	...
	public ServiceDTO selecionarTodosUsuario( ServiceDTO requestDTO ) throws LayerException
	{
		...
		if ( ( lista != null ) && ( ! lista.isEmpty( ) ) )
		{
			...
			for ( int i = 0 ; i < lista.size( ); i++ )
			{
				...
				UsuarioVO usuarioVO = new UsuarioVO( usuario.getLogin( ), usuario.getSenha( ), usuario.getGrupo( ), usuario.getPerfil( ),
											  usuario.getBloqueado( ), usuario.getUltimoAcesso( ), usuario.getNome( ) ) ;
